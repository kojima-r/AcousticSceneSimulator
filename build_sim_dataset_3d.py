...
if __name__ == "__main__":
	...
	step = fftLen/4
	scale=32767.0
	output_dir="data_sim_3d/"
	output_label_dir="label_3d/"
	deg_step=30
	alpha=0.1
	f = open("setting_3d.json")
	data = json.load(f)
	print(data)
	if len(sys.argv)<2:
	...
	for mic in data["mics"]:
		...
		for src in data["sources"]:
			...
			x=tf_config["tf"][src_index]["position"]
			print("# simulation: %s, direction of arrival: "%(wav_filename),x)
			#
		...
		if(len(recorded_wav)>0):
			...
			## save
			output_filename=output_dir+"test"+mic["name"]+".wav"
